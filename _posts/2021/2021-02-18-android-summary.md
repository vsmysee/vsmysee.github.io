...
```
src目录：放置所有java源代码
gen：R.java，BuildConfig.java (生成的文件)
assets: 用于存放需要打包到apk的静态文件，支持文件的子目录。没有资源ID
bin: 输出文件夹，apk就在这个文件夹内
jni: 当Android工程加入NDK支持的时候，有NDK生成，用于放置make file和native（c/c++）代码
obj: jni生成的中间结果
proguard：当工程enable proguard时自动生成
res：资源文件夹
AndroidMenifest.xml：Android apk清单，四大组件和权限声明（以及测试组件）等都需要在此文件中声明
...
SharedPreferencds文件是使用XML格式来对数据进行管理的。SharedPreferencds存在一些性能问题，国内微信做了一个MMKV方案。
...
早期还存在一个方案是Traceview ，但是不建议使用，建议使用性能剖析器
